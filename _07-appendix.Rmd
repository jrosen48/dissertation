...
## Research Question #1 additional materials
...
## Research Question 2 additional materials

```{r, rq2-0-tab, cache = TRUE}
...
## Research Question #3 additional materials
...
```{r, rq3-1-diff-by-ind-vars-2}
d %>% 
  select(inquiry_based_three, dm_cog_eng, dm_beh_eng, dm_aff_eng, dm_challenge, dm_competence) %>% 
  group_by(inquiry_based_three) %>% 
  summarize_all(rounded_mean) %>% 
  knitr::kable(booktabs = TRUE, caption = "Differences across variables that make up profiles by activity")
```


```{r, inquiry-separate, cache = TRUE}
...
```{r, inquiry-separate-comp, eval = FALSE, cache = TRUE}
...
## Research question #4 additional materials
