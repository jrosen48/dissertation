...
knitr::opts_chunk$set(echo = FALSE, message = F, warning =T, cache = F)
```
...
df <- left_join(df, pm, by = "program_ID")
```
...
# Sums and means by program name
...
dfs %>% 
    ...
    select(program_name, dm_ask:dm_com) %>% 
    gather(key, val, -program_name) %>% 
    ggplot(aes(x = program_name, y = val, fill = program_name)) +
    geom_col() +
    facet_wrap("key")
```
...
```{r}
# dfs %>%
#     filter(program_name == "Providence - Explore the Bay") %>%
#     select(program_ID, program_name, response_date, signal_number, contains("dm")) %>% View()

dfs %>% 
    # filter(program_name == "Providence - Explore the Bay") %>% 
    select(program_ID, program_name, response_date, signal_number, dm_ask, dm_obs, dm_gen, dm_mod, dm_com) %>% 
    arrange(program_name, response_date, signal_number) %>% 
    distinct()
```