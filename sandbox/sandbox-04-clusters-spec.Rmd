...
```{r, six-p-m-1}
m <- create_profiles_mplus(df, dm_cog_eng, dm_beh_eng, dm_aff_eng, dm_challenge, dm_competence, n_profiles=6, model = 1, return_save_data=T)
...
```{r}
cc <- m[[2]] %>% select(contains("DM")) %>% complete.cases()
x <- df %>% select(contains("dm"))
d <- bind_cols(x[cc, 6:ncol(x)], m[[2]])
```

*7 profiles, model 2*

```{r, 7-p-m-2}
m <- create_profiles_mplus(df, dm_cog_eng, dm_beh_eng, dm_aff_eng, dm_challenge, dm_competence, n_profiles=7, model = 2, return_save_data = TRUE)
plot_profiles_mplus(m)
```
