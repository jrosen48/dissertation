...
* All five variables w/ strict defaults
* All five variables standardized w/ strict defaults

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, message = F, warning = F, cache = F, error = F)
```
...
```{r, 2a}
out_df1 <- compare_models_mplus(df, dm_cog_eng, dm_beh_eng, dm_aff_eng, dm_challenge, dm_competence)
```
...
out_df1 %>% 
    ...
    filter(val != "Convergence problem",
           val != "LL not replicated") %>% 
    ggplot(aes(x = n_profiles, y = val, group = key, color = key)) +
    geom_point() +
    geom_line()
```
...
out_df1 <- compare_models_mplus(df_ss, dm_cog_eng, dm_beh_eng, dm_aff_eng, dm_challenge, dm_competence)
```
...
```{r, 2c}
out_df1 <- compare_models_mplus(df, dm_cog_eng, dm_beh_eng, dm_aff_eng, dm_challenge, dm_competence, st_iterations = 50, starts = c(500, 50))
```
...
out_df1 <- compare_models_mplus(df_ss, dm_cog_eng, dm_beh_eng, dm_aff_eng, dm_challenge, dm_competence, st_iterations = 50, starts = c(500, 50))
```
...
```{r, 2e}
out_df1 <- compare_models_mplus(df, dm_cog_eng, dm_beh_eng, dm_aff_eng, dm_challenge, dm_competence, st_iterations = 50, starts = c(500, 50), convergence_criterion=1E-7)
```
...
out_df1 <- compare_models_mplus(df_ss, dm_cog_eng, dm_beh_eng, dm_aff_eng, dm_challenge, dm_competence, st_iterations = 50, starts = c(500, 50),convergence_criterion= 1E-7)
```
