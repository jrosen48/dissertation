...
knitr::opts_chunk$set(echo = FALSE, message = F, warning =T, cache = T)
```
...
```{r, 2b}
dfs <- df %>% select(dm_cog_eng, dm_beh_eng, dm_aff_eng, dm_challenge, dm_competence) %>% mutate_all(center_and_scale_vector)

out_df2 <- compare_models_mplus(dfs, dm_cog_eng, dm_beh_eng, dm_aff_eng, dm_challenge, dm_competence)
```
