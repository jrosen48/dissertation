...
d_red %>% 
  ...
  skimr::skim() %>% 
  knitr::kable(format = "latex", booktabs = TRUE, caption = "Descriptive statistics for study variables", linesep = "") %>% 
...
overall_stats_for_all_models[[2]] %>% 
  ...
  select(-model, n_profiles = `Number of Profiles`) %>% 
  mutate(VLMR = paste_stats(VLMR_val, VLMR_p),
         LMR = paste_stats(LMR_val, LMV_p),
