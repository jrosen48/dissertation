...
Research question #2 is focused on the relations between each of the profiles and instructional support for work with data. This section first presents null models. These are followed by two findings related to a variable-centered approach, namely, correlations between individual aspects of work with data and the composite and the profiles and individual interest and correlations between individual aspects of work with data and the composite and the variables that make up the profiles and individual interest. Finally, results of mixed effects models with the work with data variables added separate and then with the composite for instructional support for work with data.
...
For the composite of work with data, the composite predicted profile 1, only behavioral (*B* = 0.008 (0.004), *p* = .019), but not any of the other profiles. Sensitivity analysis suggests However, this coefficient is very small. 
...
This question focuses on whether the relations differ on the basis on the activity students are doing. This section first describes differences across profiles by youth activity and by the variables that make up the profiles by youth activity. Next, findings from mixed effects models predicting activity, activity and the composite, and the activity interacted with the composite are presented.
...
These models show that the activity is still related to full engagement (*B* = 0.063 (0.015), *p* < .001) and that the work with data composite is related to only behavioral (*B* = 0.008 (0.004), *p* = .016). The effect of activity is robust, with 3.08% of the effect needing to be due to bias to invalidate the inference. The effect of the composite was not, with only 1.375% of the effect needing to be due to bias to invalidate the inference. These patterns were very similar to those found in earlier models with each of the predictor variables on their own.
...
The most robust finding appears to be the relation between the activity and full engagement. Otherwise, the findings suggest minimal relations. 

## Research Question #4

Research question #4 is focused on how the relationships observed as part of RQ2 differ on the basis of pre-program interest. In this section, findings from models with pre interest, composite, and activity, with pre interest interacted with the composite, and with pre interest interacted with youth activity, are presented.
