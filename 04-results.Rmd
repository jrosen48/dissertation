...
#      `Proportion of Instructional Episodes` = round(the_ns / 236, 3),
#      N = the_ns) %>%
#  mutate(total_N = 236-N) %>%
#  gather(key, val, -`Aspect of Work with Data`, -`Proportion of Instructional Episodes`) %>%
#  mutate(key = factor(key, levels = c("total_N", "N"))) %>%
#  ggplot(aes(x = `Aspect of Work with Data`, y = val, fill = key)) +
#  geom_col() +
#  scale_x_discrete(limits = c("Asking Questions", "Making Observations", "Generating Data", "Data Modeling", "Communicating Findings")) +
#  scale_fill_manual("", values = c("lightgray", "darkgreen"), labels = c("Not Present", "Present")) +
#  theme_bw() +
#  ylab("Number of Instructional Episodes") +
#  xlab(NULL) +
...
t  %>% 
 ...
 knitr::kable(format = "latex", caption = "Raw variables values for each profile", booktabs = TRUE, linesep = "") %>%
 kableExtra::kable_styling(latex_options = "scale_down") %>%
 kableExtra::landscape()
```
...
#  select(Profile, `Instructional Episode`:Program) %>% 
#  group_by(Profile) %>% 
#  mutate(total_exp = `Instructional Episode` + Youth + Program) %>% 
#  mutate(Residual = 1 - total_exp) %>% 
#  select(-total_exp) %>% 
#  ungroup() %>% 
#  gather(key, val, -Profile) %>% 
#  arrange(Profile) %>% 
#  mutate(key = factor(key, levels = rev(c("Instructional Episode", "Youth", "Program", "Residual")))) %>% 
#  ggplot(aes(x = Profile, y = val, fill = key)) + 
#  geom_col() +
#  scale_fill_brewer("", type = "qual", palette = 1, direction = -1, guide = guide_legend(reverse = T)) +
#  scale_x_discrete(limits = rev(c("Universally low (n = 667)",
#           "Only behavioral (n = 370)",
#           "Only affective (n = 345)",
#           "All moderate (n = 638)",
#           "Engaged and competent but not challenged (n = 450)",
...
These (somewhat minimal) findings for the youth characteristics were more surprising than those observed for the aspects of work with data. The results of research question #3, on the sources of variability for the profiles of engagement, suggested that there was much systematic variability at the level of the youth (there were large *ICC*s at the youth level, with smaller *ICC*s at the instructional episode level). Because pre-interest, gender, and URM status were variables at this level, it could be expected that they would have meaningful relations with the profiles of engagement. However, it appears that the particular youth characteristics considered were not useful at explaining much of this variability; possible reasons why are discussed further in the next section.
