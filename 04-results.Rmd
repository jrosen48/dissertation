...
o <- mutate(o, model = c(
  str_c("profile_", 1:6, "_null"),
  str_c("profile_", 1:6, "_1"),
  str_c("profile_", 1:6, "_2"),
  str_c("profile_", 1:6, "_3")
))
...
o %>% 
  ...
  knitr::kable(format = "latex", booktabs = TRUE, caption = "Results of mixed effects models", linesep = "") %>% 
  kableExtra::kable_styling(latex_options = "scale_down") %>% 
  group_rows("Null", 1, 6) %>% 
  group_rows("Model 1", 7, 12) %>% 
  group_rows("Model 2", 13, 18) %>% 
  group_rows("Model 3", 19, 24) %>% 
  landscape()
