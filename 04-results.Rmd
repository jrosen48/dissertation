...
```{r, rq2-int-interaction, cache = TRUE}
...
```{r, prep-df-for-rqs-2-and-3, cache = TRUE}
...
tidy_model <- function(model) {
  ...
  # ICCs
  icc_vals <- sjstats::icc(m1c) %>%
    as.numeric() %>%
    round(3)
  
  # To return
  names(icc_vals) <- str_c(names(sjstats::icc(m1c)), "_ICC")
...
o %>% 
  select(model, 
         ...
         program_ID_ICC) %>% 
  slice(7, 12) %>% 
  knitr::kable(format = "latex", booktabs = TRUE, caption = "Results of mixed effects models for model 1 (for RQ2)", linesep = "") %>% 
  kableExtra::kable_styling(latex_options = "scale_down")
...
o %>% 
  select(model, 
         ...
         program_ID_ICC) %>% 
  slice(13, 18) %>% 
  knitr::kable(format = "latex", booktabs = TRUE, caption = "Results of mixed effects models for model 2 (for RQ3)", linesep = "") %>% 
  kableExtra::kable_styling(latex_options = "scale_down")
...
o %>% 
  select(model, 
         ...
         program_ID_ICC) %>% 
  slice(19, 24) %>% 
  knitr::kable(format = "latex", booktabs = TRUE, caption = "Results of mixed effects models for null models for model 3 (for RQ3)", linesep = "") %>% 
  kableExtra::kable_styling(latex_options = "scale_down")
