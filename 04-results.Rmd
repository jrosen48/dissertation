...
p %>% 
  ...
  kableExtra::column_spec(11, width = "3em") %>% 
  kableExtra::column_spec(12, width = "3em") %>% 
...
```{r, eval = T}
o <- read_rds("data/m1d-6d-alt.rds")
o <- mutate(o, model = c(
  str_c("profile_", 1:6)))
...
names(oo) <- c("Profile", "Intercept", "Pre-interest", "Gender-Female", "URM status",                    "ssb_predict", "ssb_model", "ssb_analyze",
               "ssb_measure", "ssb_tools", "ssb_precision",
               "ssb_vocabulary", "ssb_classification", "ssb_symbols")
...
oo <- mutate_at(oo, vars(Intercept:ssb_symbols), re_p)
...
oo %>% 
  ...
  group_rows("Aspects of Work With Data", 5, 13) %>% 
