...
In this section, specific models are examined so that candidate solutions can be compared. For all of the solutions, the data are centered to have a mean equal to 0 and a standard deviation of 1 (thus, the y-axis on each of the plots is labeled "Z-score").
...
p <- m1_6 %>% 
  plot_profiles_mplus(to_scale = TRUE) +
...
m1_7 %>% 
  tidyLPA::plot_profiles_mplus(to_scale = TRUE) +
...
```{r, rq2-1-all-vars-com, cache = FALSE}
...
tidy_model <- function(model) {
  ...
  fixef_names <- c("(Intercept)", "dm_ask", "dm_ask:overall_pre_interest", "dm_com",
                   ...
                   "overall_pre_interest:dm_obs", "gender_female", "urm")
...
Next, correlations between individual aspects of work with data (and the composite) and the variables that make up the profiles are presented. These correlations suggest that the aspects of work with data are not only not related to the profiles, but also not related to the aspects of work with data (with values ranging from .00 to .06). 
...
When the predictor variables for work with data are added, some overall patterns and specific findings can be identified. The only relations with *p*-values that were below the criterion for statistical significance (.05) were for the relations between modeling data and full engagement (*B* = 0.03 (0.016), *p* = .033) and between generating data and full engagement (*B* = 0.03 (0.015), *p* = .02). Sensitivity analysis for the effect of generating data suggested that 1.47% of the inference would have to be due to bias to invalidate the inference, suggesting that this effect is not very robust to potential sources of bias, such as an omitted (in this analysis) confounding (or control) variable. For the effect of modeling, 4.939% would need to be due to bias to invalidate the inference. The effect of being female upon universally low engagement was moderate (*B* = 0.057 (0.035), p = .050).


```{r, rq2-1-tab, cache = TRUE}
...
# To sustain an inference, 4.939% of the estimate would have to be due to bias. This is based on a threshold of 0.032 for statistical significance (alpha = 0.05).
konfound::konfound(m6a,dm_mod)
# To invalidate an inference, 1.477% of the estimate would have to be due to bias. This is based on a threshold of 0.03 for statistical significance (alpha = 0.05).
...
tidy_model_com <- function(model) {
  ...
  fixef_names <- c("(Intercept)", "dm_composite", "overall_pre_interest", "overall_pre_interest:dm_composite", "gender_female", "urm")
  
  d <- as.data.frame(matrix(rep(NA, 6), ncol = 6))
  names(d) <- fixef_names
...
## Research Question #3: Relations Between Activity and PECs

Broadly, this question is focused on how the activity, in terms of whether students are doing an inquiry-based activity, relates to the profiles. Like for research question #2, for the primary results for this question, linear models that account for the cross-classification of the moment and youth are used and for the "nesting" of both within each of the nine programs are used; instead of codes for instructional support for work with data, codes for the activity are used. 

First, differences across profiles by youth activity and by the variables that make up the profiles by youth activity are described. Next, findings from mixed effects models predicting profiles on the basis of the inquiry based activity are interpreted (like for research question #1, additional materials are included in the appendix), in order to see the effect of the activity on its own (indepedent from instructional support for work with data. Next, the composite for work with data is added to these models, and these models are interpreted. Finally, models with activity interacted with the composite are presented and interpreted.
...
Mean differences across the variables that make up the profiles and youth activity are examined using *t*-tests. Of these five mean differences, there were statistically significant differences between inquiry-based and non-inquiry-based activities between all of the variables except for competence; students were more cognitively (*p* = .036, *d* = 0.09), behaviorally (*p* = .002, *d* = 0.13), and affectively (*p* < .001, *d* = .17), and were also more challenged (*p* < .00, *d* = .27)

<!-- ### Differences across the variables that make up the profiles by activity (with other and not focused activities) -->

<!-- Mean differences across the variables that make up the profiles and youth activity are examined using *t*-tests. Of these five mean differences, there were statistically significant differences between inquiry-based and non-inquiry-based activities between all of the variables except for competence; students were more cognitively (*p* = .036, *d* = 0.09), behaviorally (*p* = .002, *d* = 0.13), and affectively (*p* < .001, *d* = .17), and were also more challenged (*p* < .00, *d* = .27) -->

### Models with activity

```{r, rq3-2-inq, cache = FALSE}
...
tidy_model_inq <- function(model) {
  ...
  fixef_names <- c("(Intercept)", "inquiry_based", "dm_ask", "dm_obs", "dm_gen", "dm_mod", "dm_com", "inquiry_based:dm_ask", "inquiry_based:dm_obs", "inquiry_based:dm_gen", "inquiry_based:dm_mod", "inquiry_based:dm_com", "dm_composite", "overall_pre_interest", "gender_female", "urm")
  
  d <- as.data.frame(matrix(rep(NA, 16), ncol = 16))
  names(d) <- fixef_names
...
```{r, eval = FALSE, sens-RQ3,  cache = FALSE}
konfound::konfound(m6oi, inquiry_based)
```

These models show that the activity predicts full engagement (*B* = .027 (0.016), *p* < .001). However, this effect is associated with a very small coefficient. Sensitivity analysis suggests that 51.55% of the inference would need to be due to bias to invalidate the effect. None of the other profiles were statistically significantly related. 

```{r, rq3-2-interactions-with-work-with-data, cache = FALSE, eval = FALSE}
...
```{r, rq3-2-inq-and-composite, cache = FALSE}
...
These models show that the activity is still related to full engagement (*B* = 0.063 (0.015), *p* < .001) and that the work with data composite is related to only behavioral (*B* = 0.008 (0.004), *p* = .016). The effect of activity is robust, with 53.08% of the effect needing to be due to bias to invalidate the inference. The effect of the composite was not, with only 1.375% of the effect needing to be due to bias to invalidate the inference. These patterns were very similar to those found in earlier models with each of the predictor variables on their own.

```{r, rq-3-sens-2, eval = FALSE,  cache = FALSE}
konfound::konfound(m6iio, inquiry_based)
konfound::konfound(m1iio, dm_composite)
```

### Models with activity interacted with the composite

```{r, rq3-2-interactions-with-composite, cache = FALSE}
...
These models show a similar pattern as the model without the interaction, with statistically significant relationships between the activity and full engagement (*B* = 0.079 (0.026), *p* = .001) and between the composite and only behavioral (*B* = 0.008 (0.005), *p* = .034). There were no statistically significant effects of gender and URM status. The effect of activity was once again robust, with 34.35% of the effect needing to be due to bias to invalidate the inference; the effect of the composite on the probability of an only behavioral profile was not as robust, with 18.93% of the effect needing to be due to bias to invalidate the inference.

```{r, rq-3-sens-3, eval = FALSE,  cache = FALSE}
konfound::konfound(m6ii, inquiry_based)
konfound::konfound(m1ii, dm_composite)
```

```{r, inquiry-composite-interaction, cache = TRUE}
...
The strongest finding appears to be the relation between inquiry-based activities and full engagement; the difference in probability of full engagement was around 0.06 - 0.07, suggesting that this profile was around 6-7% more likely when learners were doing one of these activities. This effect was fairly robust to potential sources of bias. Reasons for the effect of the activity are explored further as part of research question #5 and are discussed in the next chapter. Apart from full engagement, the models presented in this section suggest minimal relations between inquiry-based activities and the profiles. Importantly, these inquiry-based activities do not necessarily involve work with data, suggesting that the activity in general may impact learners' (full) engagement, but that support for specific activities may not. In the next section, the interactive effect of the factors explored as part of research questions #2 and #3 (instructional support for work with data and the activity being inquiry-based) with learners' pre-program interest is described.

## Research Question #4: Effects of Youth's Pre-Program Interest 

Research question #4 is focused on how the relationships of instructional support for work with data and the activity differ on the basis of pre-program interest. Like for the previous two research questions, linear models that account for the cross-classification of the moment and the youth--and their nesting within the programs--are used. Findings from models with pre interest, composite, and activity are interpreted, and findings from models with pre interest interacted with the composite, and with pre interest interacted with youth activity, are presented and interpreted.
...
These models show no statistically significant relations, though the composite by pre interest interaction for the engaged and competent but not challenged profile has a p-value less than .10 (*B* = .005 (.004), *p* = .081). There were no significant relations between gender and URM status.

```{r, md-block-for-rq4, eval = TRUE, cache = TRUE}
...
These models show no statistically significant relations, though the activity by pre interest interaction for the engaged and competent but not challenged profile has a p-value less than .10 (*B* = .023 (.017), *p* = .079). There were no gender or URM status effects.
...
Research question #5 is focused on investigating specific findings of interest. To code the data, three research assistants trained for approximately eight hours over four meetings. Then, each research assistant coded all of the segments associated with one of the videos. After the coding was complete, the three research assistants and I met to discuss how well the coding frame and potential sources of disagreement. Then, two coders coded every segment that was coded for at least one of the aspects of instructional support for work with data. After each program, the coders met to discuss potential issues that emerged throughout the coding, and to clarify how they applied the coding frame. As this was open-ended coding with the aim to provide greater detail and context for the findings associated with research questions #2-#4, establishing reliability among the coders was not carried out. The coders sought to document specific instances of work with data that may have instigated the coders for instructional support for work with data to code the segment the way they did, as well as potential discrepencies between the coders' understanding of the coding frame for instructional support for work with data, and any other moments that seemed to be particularly engaging or disengaging to students. This coding resulted in around three to four sentence notes associated with each segment from each of two raters. In the remainder of this section, five findings associated with the qualitative coding are presented. Three of these findings emerged from findings associated with research questions #2-#4, whereas two emerged exclusively from the coding. 
...
This finding of interest emerged from the analyses research question #2 (as well as research question #4)

### Why is the activity related to work with data?

This finding of interest emerged from the analyses for research question #3 (as well as research question #4)

### How does support for work with data lead to specific student activities related to work with data, i.e., what do we notice with respect to generating and modeling data?

This finding of interest also emerged from the analyses research question #2 (as well as research question #4)

### What affordances and constraints do summer STEM programs have for work with data?

This finding relates to qualitative analyses for other features of interest. 

### Challenges of fostering a complete cycle of investiation in summer STEM programs not focused on work with data
