...
```{r}
df %>% 
    filter(program_name == "Boston - Sociedad Latina") %>% 
    select(sociedad_class, program_name, response_date, signal_number, dm_ask, dm_obs, dm_gen, dm_mod, dm_com) %>% 
    arrange(response_date, sociedad_class, signal_number) %>%
    distinct() %>% 
    clipr::write_clip()
```