...
d %>% 
  ...
  kableExtra::landscape() %>% 
  kableExtra::column_spec(width = "6in")
...
### Appendix F: Supplementary correlational analysis

```{r, fig.cap = "Supplementary correlational analysis"}
o <- d_red %>% 
  select(dm_ask:dm_com, dm_cog_eng:dm_competence, overall_pre_interest) %>%
  corrr::correlate() %>% 
  corrr::shave() %>%
  corrr::fashion()

names(o) <- c("", "Asking Questions", "Making Observations", "Generating Data", "Data Modeling", "Communicating Findings", "Cog. eng.", "Beh. eng.", "Aff. eng.", "Challenge", "Competence", "Pre-interest")
